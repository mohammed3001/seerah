...
export function ActionFeedback({ state }: { state: ActionState | undefined }) {
  ...
  const color = state.ok ? "text-emerald-700 bg-emerald-50 border-emerald-200" : "text-rose-700 bg-rose-50 border-rose-200";
  return (
    <div
      role="status"
      className={cn(
...
export function ActionWarning({ message }: { message: string }) {
  return (
