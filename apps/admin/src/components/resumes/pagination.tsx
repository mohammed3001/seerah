...
import { PER_PAGE_OPTIONS } from "@/lib/resumes/types";
...
function pageHref(params: URLSearchParams, page: number): string {
  ...
  return `/resumes?${next.toString()}`;
}

export function ResumesPagination({ page, perPage, total }: PaginationProps) {
        ...
        <label className="flex items-center gap-2">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              next.delete("page");
              window.location.href = `/resumes?${next.toString()}`;
            }}
